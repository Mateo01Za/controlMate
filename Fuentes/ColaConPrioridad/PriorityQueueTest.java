...
/*_______________________TESTEAMOS EL METODO size()_____________________________*/
	...
	public void size() {// Pruebo si insertar() actualiza el size()
		try {
			assertTrue(
					"Tamanio de la cola con prioridad justo despues de ser creada != 0",
					s.size() == 0);
			s.insert(i1, n1);
			assertTrue(
					"Tamanio de la cola con prioridad luego de insertar una vez != 1",
					s.size() == 1);
			s.insert(i2, n2);
			assertTrue(
					"Tamanio de la cola con prioridad luego de insertar dos veces != 2",
					s.size() == 2);
			s.insert(i3, n3);
			assertTrue(
					"Tamanio de la cola con prioridad luego de insertar tres veces != 3",
					s.size() == 3);
		...
		try {
			s.min();
			assertTrue(
					"Tamanio de la cola con prioridad cambio despues de invocar min()",
					s.size() == 3);
			s.removeMin();
			assertTrue(
					"Tamanio de la cola con prioridad luego de eliminar el minimo es != 2",
					s.size() == 2);
			s.min();
			assertTrue(
					"Tamanio de la cola con prioridad cambio despues de invocar min()",
...
/*_______________________TESTEAMOS EL METODO isEmpty()_____________________________*/
	...
	public void isEmpty() {// Pruebo al insertar(), isEmpty() funciona
		...
		try {
			assertTrue(
					"La cola con prioridad no esta vacia justo despues de ser creada",
					s.isEmpty());
			s.insert(i1, n1);
			assertTrue(
					"La cola con prioridad esta vacia justo despues de insertar 1 elemento",
					!s.isEmpty());
			s.insert(i2, n2);
			assertTrue(
					"La cola con prioridad esta vacia justo despues de insertar 2 elementos",
					!s.isEmpty());
			s.insert(i3, n3);
			assertTrue(
					"La cola con prioridad esta vacia justo despues de insertar 3 elementos",
					!s.isEmpty());
		} catch (InvalidKeyException e) {
			fail("El metodo insert() no deberia lanzar InvalidkeyException con una clave valida.");
		}
		...
		try {
			s.min();
			assertTrue(
					"La cola con prioridad esta vacia justo despues de insertar invocar min() teniendo 3 elementos",
					!s.isEmpty());
			s.removeMin();
			assertTrue(
					"La cola con prioridad esta vacia justo despues de eliminar 1 elemento, quedando 2.",
					!s.isEmpty());
			s.min();
			assertTrue(
					"La cola con prioridad esta vacia justo despues de insertar invocar min() teniendo 2 elementos",
					!s.isEmpty());
			s.removeMin();
			assertTrue(
					"La cola con prioridad esta vacia justo despues de eliminar 1 elemento, quedando 1.",
					!s.isEmpty());
			s.min();
			assertTrue(
					"La cola con prioridad esta vacia justo despues de insertar invocar min() teniendo 1 elemento",
					!s.isEmpty());
			s.removeMin();
			assertTrue(
					"La cola con prioridad no esta vacia justo despues de eliminar todos los elementos que tenia.",
					s.isEmpty());
		} catch (EmptyPriorityQueueException e) {
			e.printStackTrace();
			fail("El metodo min() o removeMin() no deberia lanzar EmptyQueueException con una cola con elementos.");
